...
#region campaigns
app.MapGet("/api/1.0/marketing/campaigns", async (StylishContext context) =>
{
...
app.MapGet("/api/1.0/products/details", async (StylishContext context, long id) =>
 {
...
#region products
app.MapGet("/api/1.0/products/{category}", async (StylishContext context, string category, int paging, string? search, long? id) =>
 {
     Product[] _products = await context.Products.ToArrayAsync();
